/**
 * Script to insert the first article (JEE Main 2026) directly into Supabase
 * Run this in the browser console or as a Node.js script
 * 
 * Usage (in browser console on your site):
 * 1. Make sure you're logged in with admin/service role permissions
 * 2. Copy and paste this entire script
 * 3. Or use: import('./scripts/insert_article.js') if running from Node.js with Supabase client
 */

import { createClient } from '@supabase/supabase-js';

const JEE_ARTICLE_CONTENT = `# JEE Main 2026 Preparation Guide: Smart Strategies for Top Ranks  
...
async function insertJEEArticle() {
  ...
  if (!supabaseKey) {
    console.error('❌ Supabase key not found. Set VITE_SUPABASE_ANON_KEY or VITE_SUPABASE_SERVICE_ROLE_KEY');
    return;
  }
  ...
  // Generate slug
  const slug = 'jee-main-2026-preparation-guide-smart-strategies-for-top-ranks';
  
  // Extract excerpt
  const excerpt = 'The Joint Entrance Examination (JEE) Main is one of the most competitive exams in India, serving as a gateway to NITs, IIITs, and as a qualifier for JEE Advanced. With lakhs of students appearing each year, cracking it requires more than just hard work—it demands strategy, consistency, and clarity.';
